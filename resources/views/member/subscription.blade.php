...
@section('description', 'Our selected movies for your mood')
...
@section('content')
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col gap-2">
            <div class="text-white text-[22px] font-semibold">
                {{ ucfirst($user_subscription->package->name) }} Package
            </div>
    ...
    <div class="flex flex-col gap-[14px] max-w-max">
        <form method="post" action="{{ route('member.transaction') }}">
            @csrf
            <input type="hidden" name="package_id" id="package_id" value="{{ $user_subscription->package_id }}">
    ...
    <div class="rounded-2xl bg-[#19152E] p-[30px] w-max">
        ...
        <form action="{{ route('member.user_premium.destroy', $user_subscription->id) }}" method="post">
            @csrf
            @method('delete')
